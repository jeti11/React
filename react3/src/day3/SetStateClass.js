...
class SetStateClass extends React.Component {
  ...
  constructor(props) {
    ...
    // 자바스크립트의 생성자 안에 this.변수명 사용 시 클래스 멤버 변수로 동작함
    // SetStateClass 의 멤버 변수로 state 를 선언, 데이터 타입은 object 타입임
  ...
  StateChange = (flag) => {
    if (flag == 'direct') {
      ...
      // 리액트는 Virtual DOM을 사용하기 때문에 state의 내용을 직접 수정하면 수정된 내용이 Virtual DOM 에만 적용되고 실제 DOM에는 적용되지 않음
    ...
    if (flag == 'setState') {
      // 클래스 멤버 변수인 state의 요소인 StateString의 값을 setState() 함수를 사용하여 수정
      // setState()를 사용하여 내용을 수정 시 Virtual DOM 에 적용된 내용을 DOM에 적용함
      console.log(`원본 내용 : ${this.state.StateString}`);
      this.setState({StateString : 'setState 로 수정하여 클릭'})
      console.log(`수정 내용 : ${this.state.StateString}`);
