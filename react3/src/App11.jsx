...
// path: 웹 브라우저에 표시되는 url 주소를 설정, 절대 경로/ 상대 경로 사용 가능, *을 사용 시 모든 페이지를 뜻함(path url을 정확하게 입력해야 접속이 가능, 주로 error 페이지 설정 시 사용함)
      ...
      <div className={'container my-4'}>
        <Routes>
          ...
          <Route path={'/pages'}>
            ...
            {/*지정한 path가 아닌 주소가 입력되었을때 <ErrorPage />로 가게 됨*/}
            <Route path={'*'} element={<ErrorPage />} />
            {/*URL 파라미터 방식으로 데이터 전달*/}
            <Route path={'profile1/:username/:age'} element={<Profile1 />} />
            {/*쿼리스트링 방식으로 데이터 전달(전통적인 get 방식), url에 속성명=값 형태로 전달*/}
